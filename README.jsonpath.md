...
(5 rows)
```
Every path may be followed by a path name using an AS clause. Path names are identifiers and must be unique (including columns . Path names are used in the PLAN clause to express the desired output plan. PLAN clause could be INNER, OUTER, UNION and CROSS, which correspond to INNER JOIN, LEFT OUTER JOIN, FULL OUTER JOIN and CROSS JOIN respectively.  If there is an explicit PLAN clause, all path names must be explicit and appear in the PLAN clause exactly once.
...
-->
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTg0MzA0MzA4XX0=
-->